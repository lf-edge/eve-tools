...
// SetTimeout sets the read and write timeouts for the vsock connection.
func (c *VSOCKConn) SetTimeout(tv *unix.Timeval) error {
	if c.fd < 0 {
		return fmt.Errorf("invalid connection file descriptor")
	}

	err := unix.SetsockoptTimeval(c.fd, unix.SOL_SOCKET, unix.SO_RCVTIMEO, tv)
	if err != nil {
		return fmt.Errorf("error setting read timeout: %v", err)
	}
	err = unix.SetsockoptTimeval(c.fd, unix.SOL_SOCKET, unix.SO_SNDTIMEO, tv)
	if err != nil {
		return fmt.Errorf("error setting write timeout: %v", err)
	}
